...
var { verificarUsuAutenticado, limparSessao, gravarUsuAutenticado,
  verificarUsuAutorizado } = require("./models/autenticacao.js");
...
router.get("/perfilFisico", verificarUsuAutorizado([1, 2, 3], "pages/restrito"), async function (req, res) {
  try {
    ...
    res.render("pages/perfilFisico", { listaErros: null, dadosNotificacao: null, valores: campos })
  } catch (e) {
    res.render("pages/perfilFisico", {
...
router.post("/perfilFisico", upload.single('imagem-perfilFisico'),
  body("nomeFisico")
  .isLength({ min: 3, max: 20 }).withMessage("Mínimo de 3 letras e máximo de 20!"),
  body("emailFisico")
  .isEmail().withMessage("Digite um e-mail válido!"),
  ...
  async function (req, res) {
    ...
    if (!erros.isEmpty()) {
      console.log(erros);
      return res.render("pages/perfilFisico", { listaErros: erros, dadosNotificacao: null, valores: req.body })
    }
    try {
      var dadosForm = {
        emailFisico: req.body.emailFisico,
      ...
      if (!resultUpdate.isEmpty) {
        if (resultUpdate.changedRows == 1) {
          ...
          res.render("pages/perfilFisico", { listaErros: null, dadosNotificacao: { titulo: "Perfil! atualizado com sucesso", mensagem: "", tipo: "success" }, valores: campos });
        }
      }
    } catch (e) {
      console.log(e)
      res.render("pages/perfilFisico", { listaErros: erros, dadosNotificacao: { titulo: "Erro ao atualizar o perfil!", mensagem: "Verifique os valores digitados!", tipo: "error" }, valores: req.body })
    }
...
router.get("/perfilJuridico", verificarUsuAutorizado([1, 2, 3], "pages/restrito"), async function (req, res) {
  try {
    ...
    let campos = {
      ...
      telJuridico: results[0].telJuridico, senhaFisico: ""
    }
    res.render("pages/perfilFisico", { listaErros: null, dadosNotificacao: null, valores: campos })
  } catch (e) {
    res.render("pages/perfilFisico", {
